...
| D9 | (manual) | 18 Jul 02:20 | same URL | ✓ | ✓ | 11/11 PASS. SECURITY: cross-lead consent-token reuse now BLOCKED (CONSENT_LEAD_MISMATCH) — verified on prod. All 5 adversarial probes hold (social-eng refusal, cross-lead, forged, scope-escalation, PII redaction). Continuous-flow + anti-spec prompt hardening live. Release candidate for recording. |
| D10 | **freeze** | 18 Jul (final) | same URL | ✓ secret configured | ✓ | 11/11 PASS. FROZEN release build for submission. Community-format README + LICENSE on main. 17 tools · 5 resources · 5 prompts · 4 widgets · advance_application fast path · cross-lead consent binding. NitroChat full run verified seamless (all 4 widgets render). NO deploys after this unless verify fails against prod. |
